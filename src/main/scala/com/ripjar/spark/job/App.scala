...
object App {
  ...
  def main(args: Array[String]) {
    val parser = new scopt.OptionParser[AppConfig]("sparkJob") {
      head("sparkJob", "1.0")
  ...
  def buildProcessing(config: StreamConfig, ssc: StreamingContext) {
    ...
    val instances : Map[String, Any] = config.instances.map(inst => {
      val (procConfig: ProcessorCfg, klass: Class[Processor]) = processors.get(inst.processId).get
  ...
  def parseJsonFile(config: AppConfig): StreamConfig = {
    val json = try {
      ...
      case e: Exception => throw SparkJobException("Cannot read JSON config file: %s".format(config.configFile.getName), SparkJobErrorType.InvalidConfig)
    }
